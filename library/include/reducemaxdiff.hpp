...
class reducemaxdiff_kernel {
	...
	using local_accessor =
		...
		reducemaxdiff_kernel(write_accessor dstPtr,
		...
		void operator()(sycl::nd_item<1> item) {
			...
			for (size_t gid = global_thread_idx; gid < N; gid += stride) {
				dataT val0 = src0[gid];
				dataT val1 = src1[gid];
				dataT element = sycl::fabs(val0 - val1);
...
//     final reduction
template <typename dataT>
dataT reducemaxdiff_async(sycl::queue funcQueue,
	...
	funcQueue.submit([&] (sycl::handler& cgh) {
		auto input1_acc = src1->template get_access<sycl::access::mode::read>(cgh);
		auto input2_acc = src2->template get_access<sycl::access::mode::read>(cgh);
		...
		cgh.parallel_for(sycl::nd_range<1>(sycl::range<1>(totalThreads),
										   sycl::range<1>(lsize)),
						 reducemaxdiff_kernel<dataT>(output_acc, input1_acc, input2_acc, reduce_local_mem, dataT(0), N));
	}

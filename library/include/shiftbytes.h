...
class shiftbytes_kernel {
	public:
	    using read_accessor =
		    sycl::accessor<dataT, 1, sycl::access::mode::read, sycl::access::target::global_buffer>;
	    using write_accessor =
		    sycl::accessor<dataT, 1, sycl::access::mode::read_write, sycl::access::target::global_buffer>;
		shiftbytes_kernel(write_accessor dstPtr,
...
template <typename dataT>
void shiftbytes_async(sycl::queue funcQueue,
    ...
    funcQueue.submit([&] (sycl::handler& cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(gsize[0], gsize[1], gsize[2]),
		                 ...
		                 shiftbytes_kernel<dataT>(dst_acc, src_acc, Nx, Ny, Nz, shx, clampV));
    });
}

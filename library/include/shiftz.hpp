...
class shiftz_kernel {
	public:
		...
		shiftz_kernel(write_accessor dstPtr,
...
template <typename dataT>
void shiftz_async(sycl::queue funcQueue,
    ...
    funcQueue.submit([&] (sycl::handler& cgh) {
        ...
        cgh.parallel_for(sycl::nd_range<3>(sycl::range<3>(gsize[0], gsize[1], gsize[2]),
		                 ...
		                 shiftz_kernel<dataT>(dst_acc, src_acc, Nx, Ny, Nz, shz, clampL, clampR));
    });
}

...
void simple_workitem_indexing(sycl::nd_item<3> item,
    ...
    thdx[gid] = item.get_local_id(0); // local thread id along x-dimension
    thdy[gid] = item.get_local_id(1); // local thread id along y-dimension
    thdz[gid] = item.get_local_id(2); // local thread id along z-dimension

...
void simple_workitem_indexing(sycl::nd_item<3> item,
    ...
    thdx[gid] = syclThreadIdx_x; // global thread id along x-dimension
    thdy[gid] = syclThreadIdx_y; // global thread id along y-dimension
    thdz[gid] = syclThreadIdx_z; // global thread id along z-dimension
    grpx[gid] = syclBlockIdx_x;  // workgroup id along x-dimension for corresponding thread
    grpy[gid] = syclBlockIdx_y;  // workgroup id along y-dimension for corresponding thread
    grpz[gid] = syclBlockIdx_z;  // workgroup id along z-dimension for corresponding thread
}
